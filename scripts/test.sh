...
then
  ...
  if [ "$TRAVIS_PULL_REQUEST" == "false" ]; then
    ...
    #mvn clean install -B -Pjacoco -Pdocker -DskipTests coveralls:report sonar:sonar -Dsonar.projectKey=servicecomb-pack \
    #  && mvn clean verify -B -f demo -Pdemo -Pdocker -DskipTests -Ddocker.useColor=false -Ddocker.showLogs \
    #  && mvn clean verify -B -f acceptance-tests -Pdemo -Pdocker -DskipTests -Ddocker.useColor=false -Ddocker.showLogs
